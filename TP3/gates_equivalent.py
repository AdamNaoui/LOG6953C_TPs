...
def x_to_cnotxcnot(codeline: str, number: int):
    ...
    if re.search('prog.x', codeline) is not None:
        new_codeline = re.sub("# number=(.*)[\n]", "", codeline)
        return re.sub('x[(]', "cx(input_qubit[" + str(help_qubit) + "],", new_codeline) + "# number=" + str(
            number) + "\n" + \
            new_codeline + "# number=" + str(number + 1) + "\n" + \
            re.sub(r'x[(]', "cx(input_qubit[" + str(help_qubit) + "],", new_codeline) + "# number=" + str(
                number + 2) + "\n"
    else:
...
def z_to_cnotzcnot(codeline: str, number: int):
    ...
    if re.search('prog.z', codeline) is not None:
        ...
        return re.sub(r'[)]', ",input_qubit[" + str(help_qubit) + "])", codeline1, count=1) + "# number=" + str(
            number) + "\n" + \
            new_codeline + "# number=" + str(number + 1) + "\n" + \
            re.sub(r'[)]', ",input_qubit[" + str(help_qubit) + "])", codeline1, count=1) + "# number=" + str(
                number + 2) + "\n"
    else:
...
def two_CNOT(tab: str, qubit_number: int, number: int):
    return tab + "prog.cx(input_qubit[1],input_qubit[0]) # number=" + str(number) + "\n" + \
        tab + "prog.cx(input_qubit[1],input_qubit[0]) # number=" + str(
